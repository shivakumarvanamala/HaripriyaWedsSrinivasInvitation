...
// with a small gold "Bride / Groom" label, then the name at display size.
...
export default function Couple({ content }) {
  ...
  return (
    <section id="couple" className="section-pad relative bg-jade jali">
      <div className="mx-auto max-w-5xl text-center">
        {/* slow glow — the couple's emblem, so a little life suits it */}
        {/* Bride & groom together under a shared garland.
            `-scale-x-100` mirrors the motif: it is drawn groom-left /
            bride-right, and on this site the CARDS are bride-left, so without
            the flip the two figures would sit opposite their own cards. The
            garland and the centre heart are symmetric, so the mirror is
            invisible other than swapping the pair. */}
        <CoupleMotif className="mx-auto h-20 w-24 -scale-x-100 text-gold" />
        ...
        {/* BRIDE FIRST — this is the bride-side invitation, so her card leads.
            The groom-side site renders the same two cards in the opposite
            order. Nothing else about the cards differs. */}
        <div className="flex flex-col items-center justify-center gap-10 md:flex-row md:items-stretch md:gap-8">
          <PersonCard person={couple.bride} delay={0} />
          ...
          <PersonCard person={couple.groom} delay={0.2} />
        </div>
      </div>
    </section>
  )
